...
export var TimeUnit;
(function (TimeUnit) {
    TimeUnit[TimeUnit["YEAR"] = 0] = "YEAR";
    TimeUnit[TimeUnit["MONTH"] = 1] = "MONTH";
    TimeUnit[TimeUnit["WEEK"] = 2] = "WEEK";
    TimeUnit[TimeUnit["DAY"] = 3] = "DAY";
    TimeUnit[TimeUnit["HOUR"] = 4] = "HOUR";
    TimeUnit[TimeUnit["MINUTE"] = 5] = "MINUTE";
    TimeUnit[TimeUnit["SECOND"] = 6] = "SECOND";
    TimeUnit[TimeUnit["MILLISECOND"] = 7] = "MILLISECOND";
})(TimeUnit || (TimeUnit = {}));
export default class DateUtil {
    static ISO2Date(iso) {
        return new Date(iso);
    }
    static date2TimeStr(date) {
        return this.date2Format(date, "HH:mm:ss");
    }
    static date2DateStr(date) {
     ...
     */
    static date2Format(date, format) {
        if (!date) {
            return "";
        }
        let ret;
     ...
     */
    static date2TsStr(date, withMills = false) {
    ...
    static isAfter(preTime, nextDate) {
        return preTime.getTime() > nextDate.getTime();
    }
    static *generateLoadDataTimeIterators(startTime, endTime, loadDataStartMinute, loadDataInterval) {
    ...
    static nearestLoadDataTime(date, loadDataStartMinute, loadDataInterval) {
    ...
    static getFullTimeValueStr(value) {
        return value < 10 ? '0' + value : value.toString();
    }
    /**
     * 指定时间内的所有冻结点
     */
    static *generateFreezeDataIteratorByTimeRange(startTime, endTime) {
     ...
     */
    static nextNearestFreezeDate(date) {
        let freezeMinute;
    ...
    static date2BcdBytesBE(date, schema) {
        const year = date.getFullYear();
        if (schema === 'DATE_TIME') {
    ...
    static bcdBytes2DateBE(bytes, schema) {
    ...
    static bcdBytes2DateStrBE(bytes, schema) {
        if (schema === 'DATE_TIME') {
            const year = (bytes[0] << 8) | bytes[1];
            const mills = (bytes[8] << 8) | bytes[9];
